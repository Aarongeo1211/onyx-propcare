        ...
        <div className="flex flex-col gap-8 lg:flex-row">
          <div className="min-w-0 flex-1">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
              ...
              <div className="mb-4 flex items-center gap-1.5 text-sm text-cream/40">
              ...
              <div className="flex items-baseline gap-3">
                ...
                <span className="text-sm text-cream/30">({formatPriceFull(property.price)})</span>
                {property.isNegotiable && (
                  <span className="rounded-full border border-cream/10 px-2 py-0.5 text-xs text-cream/40">
                    Negotiable
                  </span>
                )}
              </div>
            ...
            <div className="sticky top-24 space-y-6">
              <div className="rounded-2xl border border-cream/8 bg-onyx-900/50 p-6 backdrop-blur-sm">
                <div className="mb-4 flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full border border-cream/10 bg-onyx-800 text-lg font-display text-cream/40">
                    {property.owner.name.charAt(0)}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-cream">{property.owner.name}</p>
                    <p className="text-xs text-cream/30">Property Owner</p>
                  </div>
                </div>
                ...
                {inquirySent ? (
                  <div className="py-6 text-center">
                    ...
                    <p className="mt-1 text-xs text-cream/40">The owner will contact you soon.</p>
                  </div>
                ) : !session?.user ? (
                  <div className="py-6 text-center">
                    <MessageSquare className="mx-auto mb-3 h-8 w-8 text-cream/20" />
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-cream/10 bg-onyx-900/60 px-3 py-2 text-sm text-cream placeholder:text-cream/25 focus:border-gold/40 focus:outline-none"
                    />
                    <input
                      type="tel"
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full resize-none rounded-lg border border-cream/10 bg-onyx-900/60 px-3 py-2 text-sm text-cream placeholder:text-cream/25 focus:border-gold/40 focus:outline-none"
                    />
                    <Button
...
function StatCard({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl border border-cream/8 bg-onyx-900/30 p-4">
      <div className="mb-1 flex items-center gap-2 text-cream/30">
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 whitespace-nowrap border-b-2 px-4 py-3 text-sm font-medium transition-all ${
        active ? "border-gold text-gold" : "border-transparent text-cream/40 hover:border-cream/10 hover:text-cream/60"
      }`}
    >
      {icon}
      {label}
...
function SoilDataTab({ data }: { data: SoilData }) {
  return (
    <div className="space-y-6">
      ...
      {data.testedAt && <p className="text-xs text-cream/25">Tested on {new Date(data.testedAt).toLocaleDateString("en-IN")}</p>}
      {data.reportUrl && (
...
function DataField({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div>
      <p className="mb-1 text-xs text-cream/30">{label}</p>
  ...
  return (
    <div className="flex items-start gap-3 rounded-lg bg-onyx-900/40 p-3">
      ...
      <div>
        ...
        <p className="text-xs capitalize text-cream/40">{detail}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-onyx-950 pt-20">
      <div className="text-center">
        ...
        <p className="mb-6 text-cream/40">The property you are looking for does not exist or has been removed.</p>
