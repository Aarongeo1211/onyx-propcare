...
export function SearchBar({
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      >
        <Search className={`w-5 h-5 ml-3 text-cream/80 transition-colors ${isHero ? "group-focus-within:text-gold/60" : ""}`} />
        ...
        <input
          ...
          className={`flex-1 bg-transparent px-4 text-sm text-cream placeholder:text-cream/80 focus:outline-none font-body ${
            isHero ? "py-3" : "py-2.5"
          }`}
        />
        ...
        {showSuggestions && suggestions.length > 0 && (
          ...
          >
            {suggestions.map((suggestion, i) => (
              ...
              >
                <MapPin className="w-4 h-4 text-cream/82 flex-shrink-0" />
                <div>
                  <span className="text-cream">{suggestion.value}</span>
                  {suggestion.type === "district" && suggestion.parent && (
                    <span className="text-cream/82 ml-1.5 text-xs">{suggestion.parent}</span>
                  )}
