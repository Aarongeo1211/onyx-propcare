...
export default function FortyPlusEventsPage() {
  ...
  async function handleFilesSelected(eventId: string, e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Snapshot into a real array before touching e.target.value — clearing the
    // input's value can retroactively empty the live FileList that `files` still
    // points at, so reading files.length *after* the reset sees 0 even though it
    // was correct a line earlier. Array.from copies the File references out first.
    ...
    const fileArray = selected.slice(0, remaining);
